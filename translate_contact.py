...
faq_keys = ["myMenuContactUs", "myMenuContactUsSubtitle"]
...
def process_file(file):
    ...
    print(f"[{lang_code}] Starting Contact Us translation...")
    ...
    for key, text in en_translations.items():
        if key not in data or data[key] == text: # force translation if it equals english by accident
            if key == "myMenuContactUsSubtitle":
                # subtitle is just an email address, so we just copy it
                data[key] = text
                modified = True
                continue
...
def main():
    ...
    print("Contact Us Translation script complete.")
